...
function Dashboard() {
  ...
  const [selectedPlatform, setSelectedPlatform] = useState('facebook'); // Default to 'facebook' for testing
  const [prompt, setPrompt] = useState('');
  const [generatedContents, setGeneratedContents] = useState([]); // Array for multiple responses
  const [selectedContent, setSelectedContent] = useState(null); // Selected response for posting
  ...
  useEffect(() => {
    window.fbAsyncInit = function () {
      window.FB.init({
        appId: '80681845845763', // Replace with your Facebook App ID
        cookie: true,
        xfbml: true,
        version: 'v18.0',
      });
  ...
  const handlePromptSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/generate-content', {
        ...
        body: JSON.stringify({ prompt, platform: selectedPlatform || 'facebook' }), // Fallback to 'facebook'
      });
      ...
      setGeneratedContents(data); // Expecting an array of responses
    } catch (err) {
                    ...
                    <div className="mb-2">
                      {content.mediaType === 'image' ? (
                        <img
                          ...
                          onError={() => setError(`Failed to load media for option ${index + 1}`)}
                        />
                      ) : (
                        <video
                          src={content.mediaUrl}
        ...
        <div className="p-6 bg-black text-white rounded-lg shadow-lg min-w-[400px] max-w-[90vw] mx-auto overflow-hidden border border-gray-700">
          <div className="flex items-center gap-2">
            <textarea
              ...
              rows="3"
              placeholder="Enter your prompt here (e.g., 'Generate a happy Diwali wishing pic with happy Diwali caption and hashtag')..."
              value={prompt}
